...
class CategorizedSegmenter:
    ...
    def _init_models(self):
        ...
        if self.num_gpus > 1:
            self.grounding_model = torch.nn.DataParallel(
        ...
        self.sam_predictor = SamPredictor(sam.module if self.num_gpus > 1 else sam)

        print("模型初始化完成 (多GPU模式)" if self.num_gpus > 1 else "模型初始化完成 (单GPU模式)")
    ...
    def _process_image(self, image_path, category_name):
        ...
        # 4. SAM分割
        # 需要将predictor设置到当前GPU
        current_device = f"cuda:{torch.cuda.current_device()}" if self.num_gpus > 1 else self.device
    ...
    def process_all_categories(self):
        ...
        if self.num_gpus > 1:
            ...
            print(f"使用 {self.num_gpus} 个GPU并行处理...")

            processes = []
            for gpu_id, category in zip(self.gpu_ids, self._split_categories()):
                p = mp.Process(
                    target=self._process_category_worker,
                    args=(category, gpu_id)
                )
                p.start()
                processes.append(p)
...
def _process_category_worker(self, category, gpu_id):
    """多进程工作函数"""
    torch.cuda.set_device(gpu_id)
    self._reinit_models_for_gpu(gpu_id)  # 为当前GPU重新初始化模型
    self._process_category(category)


if __name__ == "__main__":
    # 创建并运行分割器
    segmenter = CategorizedSegmenter(gpu_ids=[0, 1])
